...
void CDSphere_QuasiCylinder::CalDistance(const DiscreteObject* pdoSlave, const DiscreteObject* pdoMaster)
{
	...
	vedo_float_t dHHb = 0.5 * pdoMaster->GetDOModel()->GetShapeAttributes().quasicylinder.height;
	...
	vedo_float_t Dap = (vCa - vCb) % vAxial;
	if (Dap < -dHHb)
	{
		Dap = -dHHb;
	}
	...
	vedo_float_t dRs = pdoSlave->GetDOModel()->GetShapeAttributes().sphere.radius;
	vedo_float_t dRc = pdoMaster->GetDOModel()->GetShapeAttributes().quasicylinder.radius;
	cInfo.dImpactDepth = dRs + dRc - vIm.length();

	if (cInfo.dImpactDepth > 0.0)
	{
		vedo_float_t dS = dRs - cInfo.dImpactDepth * dRc / (dRs + dRc);
