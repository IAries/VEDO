...
Boundary::Boundary(std::string sN, const bool* bS, const njr::Vector3d vLP, const njr::Vector3d vUP): sName(sN)
{
	for (vedo_uint_t i=0; i<3; i++)
	{
		bSwitch[i] = *(bS+i);
...
Boundary::Boundary(const bool* bS, const njr::Vector3d vLP, const njr::Vector3d vUP)
{
	sName = "Boundary";
	for (vedo_uint_t i=0; i<3; i++)
	{
...
Boundary::Boundary(std::string sN, const njr::Vector3d vLP, const njr::Vector3d vUP): sName(sN)
{
	vLowerPoint = vLP;
	vUpperPoint = vUP;
	Correct();
...
{
	vedo_float_t x_min = std::min(vLowerPoint.x(), vUpperPoint.x());
	vedo_float_t x_max = std::max(vLowerPoint.x(), vUpperPoint.x());
	vedo_float_t y_min = std::min(vLowerPoint.y(), vUpperPoint.y());
	vedo_float_t y_max = std::max(vLowerPoint.y(), vUpperPoint.y());
	vedo_float_t z_min = std::min(vLowerPoint.z(), vUpperPoint.z());
	vedo_float_t z_max = std::max(vLowerPoint.z(), vUpperPoint.z());
...
{
	vedo_float_t dX = fabs(vNewRange->x());
	vedo_float_t dY = fabs(vNewRange->y());
	vedo_float_t dZ = fabs(vNewRange->z());
	vRange.Set(dX, dY, dZ);
...
bool Boundary::InBoundary(const njr::Vector3d v, const vedo_float_t r) const
{
	if (Active())
	{
		if (bSwitch[0])
...
{
	vedo_float_t dPx = vPosition->x();
	vedo_float_t dPy = vPosition->y();
	vedo_float_t dPz = vPosition->z();
...
{
	vedo_float_t dPx = vDisplacement->x();
	vedo_float_t dPy = vDisplacement->y();
	vedo_float_t dPz = vDisplacement->z();
