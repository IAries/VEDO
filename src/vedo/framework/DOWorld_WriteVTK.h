...
public:
	...
	inline static void writeVTKDataArray
		(std::ofstream* ofpVTK, const std::string& name, std::vector<_float_t>& data, const _uint_t numOfComponent)
	{
		if (data.size() == 0 )
		{
			return;
		...
		for (_uint_t i=0; i<(_uint_t)data.size(); i++)
		{
			*ofpVTK
				<< data[i] << " "                                                                  << std::endl;
		}
...
private:
	...
	void DOWorld::WriteVTK(const std::string filename) const
	{
		...
		/// raw data arrays
		std::vector<_float_t> sphere_Positions, sphere_Velocitys, sphere_AngularVelocity;
		std::vector<_float_t> sphere_Radius, sphere_GranularTempV, sphere_GranularTempAV;
		...
		for (_uint_t u=0; u<(_uint_t)sphere_Positions.size(); u++)
		{
