...
bool DOWorld::ReadIDO(std::string filename, IactRecordTab* irtp)
{
	...
	idof.read((char*) &lnum_doml, sizeof(_uint_t));
	for (i=0; i<lnum_doml; ++i)
	{
		cDOModel.push_back(new DOModel(idof));
	}

	idof.read((char*) &lnum_iactml, sizeof(_uint_t));
	for (i=0; i<lnum_iactml; ++i)
	{
		cIactModel.push_back(new IactModel(idof));
	}

	idof.read((char*) &lnum_dos, sizeof(_uint_t));
...
void DOWorld::WriteIDO(const std::string filename) const
{
	...
	*pSystemParameter >> idof;
	...
	_uint_t lnum_doml = (_uint_t) cDOModel.size();
	idof.write((char*) &lnum_doml, sizeof(_uint_t));
	for (idoml=cDOModel.begin(); idoml!=cDOModel.end(); ++idoml)
	{
		**idoml >> idof;
	}

	_uint_t lnum_iactml = (_uint_t) cIactModel.size();
	idof.write((char*) &lnum_iactml, sizeof(_uint_t));
	for (iactml=cIactModel.begin(); iactml!=cIactModel.end(); ++iactml)
	{
		**iactml >> idof;
	}

	_uint_t lnum_dos = (_uint_t) cDOStatus.size();
	idof.write((char*) &lnum_dos, sizeof(_uint_t));
    ...
    _uint_t lnum_is = 0;
	idof.write((char*) &lnum_is, sizeof(_uint_t));
